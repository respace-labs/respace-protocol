...
describe('Member Complex', function () {
  ...
  describe('Plan', () => {
    ...
    it('Create a plan', async () => {
      ...
      await expect(
        space.connect(f.deployer).createPlan('New Plan', precision.token(0.1), 0),
      ).to.revertedWithCustomError(space, 'OwnableUnauthorizedAccount')

      const tx = await space.connect(f.user0).createPlan('New Plan', precision.token(0.1), 0)
      await tx.wait()
    ...
    it('updatePlan', async () => {
      ...
      await expect(space.connect(f.user0).updatePlan(1, 'Updated Plan', 1000n, 0, true)).to.revertedWithCustomError(
        f.member,
        'PlanNotExisted',
      )

      await expect(space.connect(f.deployer).updatePlan(1, 'Updated Plan', 1000n, 0, true)).to.revertedWithCustomError(
        space,
        'OwnableUnauthorizedAccount',
      )

      const tx = await space.connect(f.user0).updatePlan(0, 'Updated Plan', precision.token('0.002048'), 0, false)
      await tx.wait()
  ...
  it('unsubscribe case 1:subscribe 1 month, then unsubscribe all', async () => {
    ...
    await expect(unsubscribe(space, f.user4, remainAmount0)).to.revertedWithCustomError(
      f.member,
    ...
    await expect(unsubscribe(space, f.user1, 0n)).to.revertedWithCustomError(f.member, 'AmountIsZero')
